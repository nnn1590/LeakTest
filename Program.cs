...
public class Program {
	public static int Main(string[] args) {
		...
		foreach (string arg in args) {
			if (arg.ToLower().Equals("int"    )) testType |= Flags.TestType.ReturnIntFunc;
			...
			if (arg.ToLower().Equals("struct" )) testType |= Flags.TestType.TestStruct1;
		...
		Console.Error.WriteLine($"Enabled flags: {testType}");
		...
		switch (runType) {
			...
			// case Flags.RunType.NotLeakingOnly:       NotLeaking.RunStatic(testType, loopCount); break;
		}
